...
exp:  exp TOK_MAS exp
      {
        ...
        if(($1.tipo == BOOLEANO) && ($3.tipo == BOOLEANO)) {
          ...
          $$.tipo = BOOLEANO;
          $$.es_direccion = 0;
        }else if (($1.tipo == BOOLEANO) || ($3.tipo == BOOLEANO)){
    ...
    | TOK_LONGITUD TOK_IDENTIFICADOR
      {
        fprintf(out,";R4444:\t<exp> ::= ^<identificador>\n");
        simboloTabla *simbolo;
        SIMBOLO *sim_aux;
        if (ambito == GLOBAL){
          if((simbolo = buscarAmbitoGlobal(tabla,$2.lexema)) == NULL){
            printf("****Error semantico en lin %li: Acceso a variable no declarada (%s)\n",nline,$2.lexema);
            deleteTablaSimbolos(tabla);
            return -1;
          }
        }else{
          if((simbolo = buscarAmbitoLocal(tabla,$2.lexema)) == NULL){
            printf("****Error semantico en lin %li: Acceso a variable no declarada (%s)\n",nline,$2.lexema);
            deleteTablaSimbolos(tabla);
            return -1;
          }
