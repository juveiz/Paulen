...
%type <atributos> aux
%type <atributos> asignaciones_elemento
%type <atributos> asignacion_elemento
...
sentencia_simple:  asignacion {fprintf(out,";R34:\t<sentencia_simple> ::= <asignacion>\n");}
                 | asignacion_vector {fprintf(out,";R34.1:\t<sentencia_simple> ::= <asignacion_vector>\n");}
...
asignacion_vector: TOK_INIT TOK_IDENTIFICADOR TOK_LLAVEIZQUIERDA asignaciones_elemento TOK_LLAVEDERECHA
          {
            fprintf(out,";R100:\t<asignacion_vector> ::= <identificador> { <asignaciones_elemento> }\n");
            simboloTabla *simbolo;
            SIMBOLO *sim_aux;
            // Revisar
            if (ambito == GLOBAL){
            ...
            strcpy($4.lexema,sim_aux->identificador);
            $4.tipo = sim_aux->tipo;
            $4.valor_entero = sim_aux->longitud;
            cont = 0;
          }
asignaciones_elemento: asignacion_elemento
            {
              fprintf(out,";R101:\t<asignaciones_elemento> ::= <asignacion_elemento>\n");
              strcpy($1.lexema,$$.lexema);
              $1.tipo = $$.tipo;
              $1.valor_entero = $$.valor_entero;
            }
           | asignacion_elemento TOK_PUNTOYCOMA asignaciones_elemento
            {
              fprintf(out,";R102:\t<asignaciones_elemento> ::= <asignacion_elemento> <asignaciones_elemento>\n");
              strcpy($1.lexema,$$.lexema);
              $1.tipo = $$.tipo;
              $1.valor_entero = $$.valor_entero;
              strcpy($3.lexema,$$.lexema);
              $3.tipo = $$.tipo;
              $3.valor_entero = $$.valor_entero;
            }

asignacion_elemento: exp
          {
              if (cont >= $$.valor_entero){
                printf("****Error semantico en lin %li: Lista de inicializacion de longitud incorrecta\n",nline);
                deleteTablaSimbolos(tabla);
                return -1;
              }
              char valor[MAX_INT];
              sprintf(valor,"%d",cont);
              escribir_operando(out,valor,0);
              escribir_elemento_vector(out,$$.lexema,$$.valor_entero,0);
              cont ++;
          }
